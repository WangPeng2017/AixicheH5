<template>
  <div class="view">
    <x-header :left-options="{backText: '', preventGoBack: true}" class="vux-1px-b" left-options.preventGoBack="true" @on-click-back="back">收银台</x-header>
    <div class="content">
      <div class="price"><em>￥</em>{{payPrice.toFixed(2)}}</div>

      <group>
        <radio :options="options" v-model="payType">
          <template slot-scope="props" slot="each-item">
            <p>
              <i class="wx-icon inline-icon" v-if="props.index===0"></i>
              <i class="zfb-icon inline-icon" v-if="props.index===1"></i>
              {{ props.label }}
            </p>
          </template>
        </radio>
      </group>
      <a href="javascript:void(0)" class="payBtn" @click="pay">确认支付</a>
    </div>
  </div>
</template>
...
<script>
  ...
  import { createGoodsCashPay, createPackageCashPay } from '@api'
    ...
    methods: {
      ...
      async pay () {
        ...
        if (this.$route.query.type === 'goods') {
          let res = await createGoodsCashPay(form)
