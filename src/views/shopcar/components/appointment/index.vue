  ...
  <div class="panel">
    ...
    <transition name="translate">
      <div class="wrapper" v-show="status">
        ...
        <tab>
          <tab-item v-for="(item, index) in dateList" :key="index" :selected="item.status" @on-item-click="tabChange">{{item.dateName + '('+ item.weekName + ')'}}</tab-item>
        </tab>
        <div class="content">
          <div class="grid">
            <div class="grid-item" :class="{on: item.status, disabled: item.is_disabled}" v-for="item in timelotList" @click="timelotChange(item)">
...
<style lang="scss" scoped>
  ...
  .panel {
    ...
    .wrapper {
      ...
      .content {
        width: 100%;
        height: calc(100% - #{$default_navbar_height} - #{$default_toolbar_height} - #{$default_toolbar_height});
        overflow: hidden;
        overflow-y: auto;
        background-color: #fff;
        .grid {
