<template>
  <div class="view">
    <x-header :left-options="{backText: ''}" class="vux-1px-b">个人求职</x-header>
    <div class="content">
      <div class="mescroll" id="job_mescroll">
        <div class="scroll">
          <job-card v-for="item in data" :data="item" :key="item.article_id"></job-card>
        </div>
        <div id="job_noData"></div>
      </div>
    </div>
    <fixed-bot-btn @click="$router.push('/myJob')">我的求职</fixed-bot-btn>
  </div>
</template>
      ...
      initMeScroll () {
        ...
        this.$nextTick(() => {
          if (this.meScroll) {
            ...
            this.meScroll = new MeScroll('job_mescroll', {
              up: {
                callback (page) {
