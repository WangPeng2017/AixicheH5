<template>
  <div class="view">
    <x-header :left-options="{backText: ''}" class="vux-1px-b">二手车</x-header>
    <div class="content">
      <div class="mescroll" id="rentcar_mescroll">
        <div class="scroll">
          <usedcar-card v-for="item in data" :data="item" :key="item.article_id"></usedcar-card>
        </div>
        <div id="rentcar_noData"></div>
      </div>
    </div>
    <fixed-bot-btn @click="$router.push('/myUsedcar')">我的二手车</fixed-bot-btn>
  </div>
</template>
