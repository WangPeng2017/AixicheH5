<template>
  <div class="view">
    ...
    <div class="content">
      <div class="mescroll"
           id="mytransfer_mescroll">
        <div class="scroll">
          <transfer-card v-for="item in data"
                         :data="item"
                         :key="item.article_id">
            <div slot="footer"
                 class="bottom"
        ...
        <div id="mytransfer_noData"></div>
      </div>
    </div>
    <fixed-bot-btn @click="gotoDetail()">发布转让信息</fixed-bot-btn>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initMeScroll () {
      ...
      this.$nextTick(() => {
        if (this.meScroll) {
          ...
          this.meScroll = new MeScroll('mytransfer_mescroll', {
            up: {
              callback (page) {
    ...
    async editArticle (item) {
      this.$router.push({
        path: '/myTransferForm/' + item.article_id,
        query: {
          channel_code: this.postData.channel_code
        }
      })
