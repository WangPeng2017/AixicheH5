    ...
    <transition name="translate">
      <div class="wrapper"
        ...
        <div class="title">
          ...
          <div class="center">
            <p class="price">
              <em class="jifen-icon"></em>&nbsp;{{goods.pointslevel }}<em class="redprice"
                  v-if="goods.pricelevel"> + ￥{{goods.pricelevel.toFixed(2)}}</em>
            </p>
            <p class="stock"
...
export default {
  ...
  methods: {
    ...
    numChange () {
      if (this.goods.purchase_limit === 0) {
        if (this.goods.goods_buynum > this.goods.goods_storage) {
          this.goods.goods_buynum = this.goods.goods_storage
        }
      } else {
        if (this.goods.goods_buynum > this.goods.purchase_limit) {
          this.goods.goods_buynum = this.goods.purchase_limit
          return this.$toasted.show('此商品单次限购数量为' + this.goods.purchase_limit + '个')
        }
      }
    },
    addShoppingCart () {
