<template>
  <div class="view">
    ...
    <data-sort :data="sortData" :enableSwitch="true" :enableFilter="false" :display="display" @sortChange="sortChange" @switchChange="changeDisplay"></data-sort>
    <div class="content">
      <div class="mescroll" id="coupongoods_mescroll">
        <div class="scroll">
          <goods-card v-for="item in data" :key="item.goods_id" :data="item" :type="display"></goods-card>
        </div>
        <div id="coupongoods_noData"></div>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  import { getGoodsListForCoupon } from '@api'
  ...
  export default {
    ...
    methods: {
      ...
      async loadGoods (page) {
        ...
        let res = await getGoodsListForCoupon(qs.stringify(this.postData))
      ...
      initMeScroll () {
        ...
        this.$nextTick(() => {
          if (this.meScroll) {
            ...
            this.meScroll = new MeScroll('coupongoods_mescroll', {
              up: {
                callback (page) {
                  self.loadGoods(page)
                },
