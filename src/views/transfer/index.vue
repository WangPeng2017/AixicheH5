<template>
  <div class="view">
    <x-header :left-options="{backText: ''}" class="vux-1px-b">转让信息</x-header>
    <tab custom-bar-width="80px">
      <tab-item v-for="(item, index) in tabData" :key="index" :selected="item.id===postData.category_id" @on-item-click="tabChanged">{{item.text}}</tab-item>
    </tab>
    <div class="content">
      <div class="mescroll" id="transfer_mescroll">
        <div class="scroll">
          <transfer-card v-for="item in data" :data="item" :key="item.article_id"></transfer-card>
        </div>
        <div id="transfer_noData"></div>
      </div>
    </div>
    <fixed-bot-btn @click="$router.push('/myTransfer')">我的转让</fixed-bot-btn>
  </div>
</template>

<script>
  import { Tab, TabItem, XHeader } from 'vux'
  import transferCard from '@comps/card/articleCard/transfer'
      ...
      initMeScroll () {
        ...
        this.$nextTick(() => {
          if (this.meScroll) {
            ...
            this.meScroll = new MeScroll('transfer_mescroll', {
              up: {
                callback (page) {
