  ...
  <div class="subview"
    ...
    <div id="goodsrecommend_mescroll"
      ...
      <div class="goods-info"
        ...
        <p class="goods-price">
          <span class="price">
            <em class="jifen-icon"></em>&nbsp;<em v-if="istoken">&nbsp;未登录！</em><em v-if="data.ispointslevel">当前等级不支持兑换！</em>{{data.pointslevel }}<em class="redprice"
                v-if="data.ispricelevel"> + ￥{{data.pricelevel.toFixed(2)}}</em>
          </span>
          <span class="expire"
      ...
      <div class="goods-promotion">
        <points-promotion-item :goods="data"
                               ...
                               :activityData="simpleActivityData"></points-promotion-item>
      </div>
      <!-- 商品或多规格服务显示规格项 -->
      <div class="goods-spec"
           v-if="(data.goods_saletype===2 && data.is_enablespec===1) || data.goods_saletype===1">
        <points-spec-item :goods="data"
                          @specShow="$refs.specPanel.show()"></points-spec-item>
      </div>
    ...
    <points-spec-panel :specData="specData"
                       :goods="data"
                       ref="specPanel"
                       @updatePriceAndStock="updatePriceAndStock"
                       @appointShow="$refs.appointPanel.show()"
                       @addShoppingCart="addShoppingCart"
                       @buy="buy"></points-spec-panel>
    <points-package-panel :packageData="packageData"
                          ref="packagePanel">
    </points-package-panel>
    <points-appoint-panel :goods="data"
                          ref="appointPanel"></points-appoint-panel>
  </div>
</template>
...
import { getGoodsPointsDetail, getGoodsImageList, getGoodsSkuSpecList, getCouponList, getGoodsPackageList, getActivityListForGoods, addShoppingCart, getGoodsListForTag } from '@api'
import sellerCard from '@comps/card/sellerCard/navigation'
import pointsPromotionItem from './components/pointsPromotionItem.vue'
import pointsSpecItem from './components/pointsSpecItem.vue'
import pointsPackagePanel from './components/pointsPackagePanel.vue'
import pointsActivityPanel from './components/pointsPromotionPanel.vue'
import pointsSpecPanel from './components/pointsSpecPanel.vue'
import pointsAppointPanel from './components/pointsAppointPanel.vue'
import qs from 'qs'
...
export default {
  ...
  methods: {
    ...
    async buy () {
      ...
      if (this.data.ispointslevel || this.istoken) {
        this.$toasted.show('无法兑换！')
        return false
      }
      let goods = this.data
      let valuenames = null
    ...
    async loadGoods () {
      ...
      if (this.token) {
        levelid = this.user.account_level_id
      }
      let res = await getGoodsPointsDetail(this.$route.params.id, lng, lat, levelid)
      this.setLoading(false)
      if (res && res.code === 200) {
        this.data = res.data
