<template>
  <div class="view">
            ...
            <div class="goods-info">
              ...
              <p class="goods-price-num">
                ...
                <template v-if="goods.package_order_id === null && goods.receive_coupon_id === null">
                  <span class="goods-price"><em>￥</em>{{goods.goods_promotionprice.toFixed(2)}}</span>
                </template>
                <!-- 使用套餐 -->
                <template v-if="goods.package_order_id !== null && goods.receive_coupon_id === null">
                  <!-- 使用购物卡 -->
    ...
    <footer class="footer">
      <span>共{{ totalCount }}{{order_type===1 ? '件' : '次'}}，合计：<em>￥{{ totalPayPrice }}</em></span>
...
import { getDefaultAddress, addOrder, getParamValue, getOrderFreightFeeList, getOrderIndoorFeeList, getOrderExpressList, getOrderTimelotList, getOrderCouponList, getOrderPackageList } from '@api'
import Vue from 'vue'
...
export default {
  ...
  filters: {
    ...
    sellerTotalPrice (item) {
      ...
      item.goods.map(g => {
        // 促销价 * 购买数量
        price += g.goods_promotionprice * g.goods_num
      })
  ...
  methods: {
    ...
    async buy () {
      ...
      this.sellers.map(seller => {
        ...
        seller.goods.map(goods => {
          ...
          let goodsPayPrice = goods.goods_payprice       // 商品的实付总价，积分兑换商品实付0
          let goodsObj = {
            ...
            point_settlement_ratio: goods.point_settlement_ratio,                // 商品手续费比例
        ...
        let orderObj = {
          ...
          goods_points: 0,                                                             // 订单商品类型： 0正常商品 1积分商品
          goods: goodsList
        }
      ...
      let res = await addOrder(qs.stringify(postData))
    ...
    async loadFreightFees (seller) {
      ...
      let res = await getOrderFreightFeeList(qs.stringify(postData))
      this.setLoading(false)
      if (res && res.code === 200) {
    ...
    async loadIndoorFees (seller) {
      ...
      let res = await getOrderIndoorFeeList(qs.stringify(postData))
      this.setLoading(false)
      if (res && res.code === 200) {
        let shipObj = res.data.find(i => i.seller_id === seller.seller_id)
        if (shipObj) {
    ...
    async loadExpresss (seller) {
      ...
      let res = await getOrderExpressList(qs.stringify(postData))
      this.setLoading(false)
      if (res && res.code === 200) {
        if (!res.data) {
          return
