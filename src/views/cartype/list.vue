<template>
  <div class="view">
    <x-header :left-options="{backText: ''}"
              class="vux-1px-b">汽车品牌</x-header>
    <div class="content">
      <div class="mescroll"
           id="carbrand_mescroll">
        <div class="scroll">
...
export default {
  ...
  methods: {
    ...
    initMeScroll () {
      ...
      this.$nextTick(() => {
        if (this.meScroll) {
          ...
          this.meScroll = new MeScroll('carbrand_mescroll', {
            up: {
              callback (page) {
    ...
    onClick (item) {
      this.$router.push({
        path: '/cartypeListForSeries/' + item.id + '/' + item.name,
