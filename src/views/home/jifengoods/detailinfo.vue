<template>
  <div class="subview" v-if="data.goods_id">
    <div id="goodsrecommend_mescroll" class="subcontent mescroll">
      <swiper :options="swiperOption">
        <swiper-slide v-for="(item, index) in imageData" :key="`swiper_${index}`">
          <img :src="item.image_url" alt="" v-imgField>
        </swiper-slide>
        <div class="swiper-pagination" slot="pagination"></div>
      </swiper>
      <div class="goods-info" v-if="data">
        <p class="goods-name" :class="{self : data.is_self}"><i class="self" v-if="data.is_self">自营</i>{{ data.goods_name }}</p>
        <p class="goods-summary" v-if="data.goods_summary != null">{{ data.goods_summary }}</p>
        <p class="goods-price">
          <span class="price">
              <em>￥</em>{{data.goods_saleprice }}
          </span>
          <span class="expire" v-if="data.goods_saletype === 2 && data.goods_validtype === 1">有效期：截止至{{getDateFullName(data.goods_validdate)}}前有效</span>
          <span class="expire" v-if="data.goods_saletype === 2 && data.goods_validtype === 2">有效期：购买后{{data.goods_validdays}}天内有效</span>
        </p>
        <div class="goods-ship" v-if="data.is_enableselfget === 1 || data.is_enableappoint === 1 || data.is_enablevisit === 1">
          <span class="tag" v-if="data.is_enableselfget === 1"><i class="info-icon"></i>支持上门自提</span>
          <span class="tag" v-if="data.is_enableappoint === 1"><i class="info-icon"></i>支持预约</span>
          <span class="tag" v-if="data.is_enablevisit === 1"><i class="info-icon"></i>支持上门服务</span>
        </div>
      </div>
<!--商家营业执照begin-->
      <div class="goods-info" v-if="data">
        <p class="goods-name" :class="{self : data.is_self}"><i class="self" v-if="data.is_self"></i>营业执照</p>
        <img/>
      </div>
<!--商家营业执照end-->
      ...
      <div class="goods-promotion">
        <goods-promotion-item
          ...
          :activityData="simpleActivityData"
        ></goods-promotion-item>
      </div>
      <!-- 商品或多规格服务显示规格项 -->
      <div class="goods-spec" v-if="(data.goods_saletype===2 && data.is_enablespec===1) || data.goods_saletype===1">
        <goods-spec-item
          :goods="data"
          @specShow="$refs.specPanel.show()"
        ></goods-spec-item>
      </div>

      <seller-card class="seller-card" :data="data"></seller-card>

      <!--热门推荐-->
      <div class="recommend" v-show="recommendGoods.length > 0">
        <divider class="divider">为您推荐</divider>
        <div class="recommend-wrapper">
          <div class="scroll">
            <goods-card v-for="item in recommendGoods" :key="item.goods_id" :data="item" :type="false"></goods-card>
          </div>
...
<script>
  ...
  import jfgoodsPromotionItem from './components/jfgoodsPromotionItem.vue'
  import jfgoodsSpecItem from './components/jfgoodsSpecItem.vue'
  import jfgoodsPackagePanel from './components/jfgoodsPackagePanel.vue'
  import jfgoodsActivityPanel from './components/jfgoodsPromotionPanel.vue'
  import jfgoodsSpecPanel from './components/jfgoodsSpecPanel.vue'
  import jfgoodsAppointPanel from './components/jfgoodsAppointPanel.vue'
  import qs from 'qs'
  ...
  export default {
    components: {
      Divider,
      jfgoodsPromotionItem,
      jfgoodsSpecItem,
      jfgoodsPackagePanel,
      jfgoodsActivityPanel,
      jfgoodsSpecPanel,
      jfgoodsAppointPanel,
      sellerCard,
      goodsCard
