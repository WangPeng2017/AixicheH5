<template>
  <div class="view">
    <x-header :left-options="{backText: ''}"
              class="vux-1px-b">我的二手车</x-header>
    ...
    <div class="content">
      <div class="mescroll"
           id="myusedcar_mescroll">
        <div class="scroll">
          <usedcar-card v-for="item in data"
                        :data="item"
                        :key="item.article_id">
            <div slot="footer"
                 class="bottom"
        ...
        <div id="myusedcar_noData"></div>
      </div>
    </div>
    <fixed-bot-btn @click="gotoDetail()">发布二手车信息</fixed-bot-btn>
  </div>
</template>

<script>
import { Tab, TabItem, XHeader } from 'vux'
import usedcarCard from '@comps/card/articleCard/usedcar'
...
export default {
  ...
  methods: {
    ...
    initMeScroll () {
      ...
      this.$nextTick(() => {
        if (this.meScroll) {
          ...
          this.meScroll = new MeScroll('myusedcar_mescroll', {
            up: {
              callback (page) {
    ...
    async editArticle (item) {
      this.$router.push({
        path: '/myUsedcarForm/' + item.article_id,
        query: {
          channel_code: this.postData.channel_code
        }
      })
