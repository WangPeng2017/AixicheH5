...
import goodsCard from '@comps/card/goodsCard/jifengoodscard'
import { getGoodsListFilters, getGoodsList, getCategoryListForLayer123 } from '@api'
...
export default {
  ...
  methods: {
    ...
    async loadGoods (page) {
      ...
      let res = await getGoodsList(qs.stringify(this.postData))
      this.setLoading(false)
      if (res && res.code === 200) {
        if (page.num === 1) {
          this.data = []
